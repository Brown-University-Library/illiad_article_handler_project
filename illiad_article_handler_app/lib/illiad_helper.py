...
class IlliadUserManager( object ):
    ...
    def check_illiad_status( self, auth_id ):
        ...
        url = '%s%s' % ( settings.ILLIAD_API_URL_ROOT, 'check_user/' )
        params = { 'user': auth_id }
        try:
            header_dct = { 'user-agent': settings.ILLIAD_API_USER_AGENT }
            r = requests.get( url, params=params, auth=(settings.ILLIAD_API_BASIC_AUTH_USER, settings.ILLIAD_API_BASIC_AUTH_PASSWORD), headers=header_dct, verify=True, timeout=10 )
            # rspns_dct = r.json()
    ...
    def setup_create_user( self, usr_dct ):
        ...
        success_check = False
        url = '%s%s' % ( settings.ILLIAD_API_URL_ROOT, 'create_user/' )
    ...
    def check_illiad_type( self, usr_dct ):
        ...
        url = '%s%s' % ( settings.ILLIAD_API_URL_ROOT, 'update_status/' )
