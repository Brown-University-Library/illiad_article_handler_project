...
from illiad_article_handler_app.lib.illiad_helper import IlliadUserManager
...
#     ( user_dct, err ) = shibber.prep_shib_dct( request.META, request.get_host() )
#     if err:
#         problem_response = handler_helper.create_problem_response( err ); return problem_response
#     ( is_new_user, err ) = illiad_user_manager.check_new_user_status( user_dct['auth_id'] )
#     if err:
...
#     if is_new_user:
#         ( create_result, err ) = new_user_helper.create_new_user( user_dct )
#         if err:
#             problem_response = handler_helper.create_problem_response( err, request ); return problem_response
...
def handler(request):
    ...
    if err:
        problem_response = handler_helper.create_problem_response( err ); return problem_response
    err = illiad_user_manager.manage_illiad_user_check( user_dct )
    if err == 'problem with ILLiad status':  # note: if err is 'problem with ILLiad new-user check' -- flow continues
        problem_response = handler_helper.create_problem_response( err, request ); return problem_response
    ## redirect to ILLiad form ----------------------------
    ( redirect_url, err ) = handler_helper.create_illiad_redirect_url( params_query_dict_copy )
    if err:
